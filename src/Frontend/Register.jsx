...
export default function RegistrationPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitted(true); // Show congrats immediately
    ...
    try {
      ...
      const resultJson = await response.json(); // parse JSON

      if (resultJson.status !== "success") {
        // If server returns error, revert submitted state
        setSubmitted(false);
        const message = typeof resultJson.message === "string"
          ? resultJson.message
    ...
    <PageWrapper>
      ...
      {!submitted ? (
        ...
        <CongratsBox>
          ...
          <CongratsText>
            You have successfully registered on <span style={{ color: "yellow", fontWeight: "bold" }}>Pinnacle 25</span> for <strong>{eventName}</strong>.
          </CongratsText>
        </CongratsBox>
      )}
    </PageWrapper>
